  ...
  template: `
    <div class="flex items-center justify-start px-4 py-3 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800">
      <div class="flex items-center gap-3">
        <app-avatar [src]="user.avatar" [alt]="user.name" [size]="'md'"></app-avatar>
        <div class="flex flex-col">
          <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100">{{ user.name }}</h2>
          <div class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300">
            <span *ngIf="user.isOnline" class="inline-flex items-center">
              <span class="w-2 h-2 bg-green-500 rounded-full mr-1"></span>
              <span translateLabel="Online"></span>
            </span>
            <span *ngIf="!user.isOnline && user.lastSeen" class="inline-flex items-center text-gray-500 dark:text-gray-400">
              <span class="w-2 h-2 bg-gray-400 rounded-full mr-1"></span>
              <span translateLabel="Last seen"></span>: {{ formatLastSeen(user.lastSeen) }}
            </span>
            <span *ngIf="!user.isOnline && !user.lastSeen" class="inline-flex items-center text-gray-500 dark:text-gray-400">
              <span class="w-2 h-2 bg-gray-400 rounded-full mr-1"></span>
